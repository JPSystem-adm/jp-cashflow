...
type SaldoRowUI = {
  ...
  Fonte: string;
  Tipo: string;
  valorInicial: number; // Saldo.valor
  // ✅ movimento (saídas - entradas) vindo da API como valorPeriodo
  valorPeriodo: number;
  // ✅ saldo atual: saldoInicial - movimento
  saldoAtual: number;
};
...
function normalizeRow(raw: unknown): SaldoRowUI | null {
  ...
  // ✅ saldo inicial: campo valor da tabela Saldo
  const valorInicial =
    toNumber(r.valorInicial, NaN) ||
    toNumber(r.saldoInicial, NaN) ||
    toNumber(r.valor, NaN) ||
  ...
  // ✅ movimento: saídas - entradas (a API deve mandar isso em valorPeriodo)
  const movimento = toNumber(
    r.valorPeriodo ?? r.movimento ?? r.movLiquido ?? r.movimentacao ?? 0,
    0
  );

  // ✅ saldoAtual = saldoInicial - movimento
  const saldoAtualFromApi = toNumber(r.saldoAtual, NaN);
  const saldoAtual = Number.isFinite(saldoAtualFromApi)
    ? saldoAtualFromApi
    : (Number.isFinite(valorInicial) ? valorInicial - movimento : 0);
  ...
  return {
    ...
    Tipo,
    valorInicial: Number.isFinite(valorInicial) ? valorInicial : 0,
    valorPeriodo: movimento,
    saldoAtual: Number.isFinite(saldoAtual) ? saldoAtual : 0,
    FonteId: Number.isFinite(FonteId) ? Math.trunc(FonteId) : 0,
  };
}

function calcMovimento(movimento: number): number {
  // ✅ movimento já vem pronto: saídas - entradas
  return movimento;
}
        ...
        <TableBody>
          {normalizedRows.length === 0 ? (
            ...
            normalizedRows.map((item, index) => {
              ...
              // ✅ Se movimento = saídas - entradas:
              // positivo => "saiu mais do que entrou" (impacta saldo pra baixo)
              // negativo => "entrou mais do que saiu"
              const movimentoClass =
                movimento > 0
                  ? "text-red-700"
                  : movimento < 0
